...
/**
 * Dashboard screen: 6 stat cards + activity feed + simple bar chart.
 */
public class DashboardView extends JPanel {
    ...
    private JPanel buildHeader() {
        ...
        JLabel date = new JLabel("Hôm nay: "
    ...
    private JPanel buildStatCards() {
        ...
        lblFarms    = addStatCard(row, "🏡", "Trang trại",       "—", AppTheme.PRIMARY);
        lblLots     = addStatCard(row, "🌾", "Lô sản xuất",      "—", AppTheme.INFO);
        lblLowStock = addStatCard(row, "📦", "Vật tư sắp hết",   "—", AppTheme.WARNING);
        lblPests    = addStatCard(row, "🐛", "Cảnh báo sâu bệnh","—", AppTheme.DANGER);
        lblHarvest  = addStatCard(row, "🌽", "Thu hoạch sắp tới","—", new Color(0x9B72CF));

        return row;
    }

    /** Creates a stat card, wires its value label, returns the value JLabel. */
    private JLabel addStatCard(JPanel parent, String icon, String title,
                               String initVal, Color accent) {
        ...
        parent.add(card);
        return valL;   // caller keeps the reference to update the value
    }
    ...
    private JPanel buildAlertPanel() {
        ...
        JLabel header = new JLabel("⚠️  Cảnh báo cần xử lý");
        ...
        // Populate alert list safely
        try {
            List<AgriSupplyDTO> low = supplyCtrl.getLowStockSupplies();
            for (AgriSupplyDTO s : low) {
                model.addElement("📦  " + s.getName() + "  còn " + s.getStockQty() + " " + s.getUnit());
            }
        ...
        try {
            ...
            for (var p : pests) {
                ...
                model.addElement("🐛  Sâu bệnh nghiêm trọng – Lô #" + lotInfo);
            }
        } catch (Exception ignored) {}

        if (model.isEmpty()) {
            model.addElement("✅  Không có cảnh báo nào");
        }
    ...
    private JPanel buildChartPanel() {
        ...
        JLabel header = new JLabel("📈  Năng suất theo mùa vụ (kg)");
        header.setFont(AppTheme.FONT_SUBTITLE);
        header.setForeground(AppTheme.TEXT_PRIMARY);
    ...
    static class AlertCellRenderer extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value,
            ...
            if (text.startsWith("📦")) lbl.setForeground(new Color(0xB45309));
            else if (text.startsWith("🐛")) lbl.setForeground(AppTheme.DANGER);
            else if (text.startsWith("✅")) lbl.setForeground(AppTheme.SUCCESS);
    ...
    // ── Inner: Simple Bar Chart ───────────────────────────────

    static class SimpleBarChart extends JPanel {
        // Sample / placeholder data — replace with real controller call
        private final String[] labels = {"Mùa Xuân", "Hè Thu", "Thu Đông", "Đông Xuân"};
